...
/**
 * This class is responsible for creating/renewing and evicting a <em>lease</em> for a particular
 * instance.
 *
 * <p>Leases determine what instances receive traffic. When there is no renewal request from the
 * client, the lease gets expired and the instances are evicted out of {@link
...
public interface LeaseManager<T> {
  ...
  /**
   * Cancel the {@link Lease} associated w/ the passed in <code>appName</code> and <code>id</code>.
   *
  ...
  /**
   * Renew the {@link Lease} associated w/ the passed in <code>appName</code> and <code>id</code>.
   *
   * @param id - unique id within appName

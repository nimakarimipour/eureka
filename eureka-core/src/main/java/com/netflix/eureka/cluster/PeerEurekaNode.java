 ...
 * this node.
 *
 * <p>This class handles replicating all update operations like <em>Register,Renew,Cancel,Expiration
 * and Status Changes</em> to the eureka node it represents.
 *
 * <p>
 *
...
public class PeerEurekaNode {
  ...
  /* For testing */ PeerEurekaNode(
      ...
      long serverUnavailableSleepTimeMs) {
  ...
  /**
   * Sends the registration information of {@link InstanceInfo} receiving by this node to the peer
   * node represented by this class.
   *
   * @param info the instance information {@link InstanceInfo} of any instance that is send to this
   *     instance.
   * @throws Exception
   */
  ...
  /**
   * Send the heartbeat information of an instance to the node represented by this class. If the
   * instance does not exist the node, the instance registration information is sent again to the
   * peer node.
   *
   * @param appName the application name of the instance.
   * @param id the unique identifier of the instance.
   ...
   *
   * <p>ASG (Autoscaling group) names are available for instances in AWS and the ASG information is
   * used for determining if the instance should be registered as {@link InstanceStatus#DOWN} or
   * {@link InstanceStatus#UP}.
   *
  ...
  /**
   * Synchronize {@link InstanceInfo} information if the timestamp between this node and the peer
   * eureka nodes vary.
   */
    ...
    try {
      if (infoFromPeer != null) {
        ...
        if (infoFromPeer.getOverriddenStatus() != null
            && !InstanceStatus.UNKNOWN.equals(infoFromPeer.getOverriddenStatus())) {
          logger.warn(
              "Overridden Status info -id {}, mine {}, peer's {}",
              id,
              info.getOverriddenStatus(),
  ...
  private static int getLeaseRenewalOf(InstanceInfo info) {
    return (info.getLeaseInfo() == null
            ? Lease.DEFAULT_DURATION_IN_SECS
            : info.getLeaseInfo().getRenewalIntervalInSecs())
        * 1000;
  }
}

...
class ReplicationTaskProcessor implements TaskProcessor<ReplicationTask> {
  ...
  public ProcessingResult process(ReplicationTask task) {
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Replication task {} completed with status {}, (includes entity {})",
            task.getTaskName(),
            statusCode,
            entity != null);
      }
      ...
      if (maybeReadTimeOut(e)) {
        logger.error(
            ...
            e);
        // read timeout exception is more Congestion then TransientError, return Congestion for
        // longer delay
        return ProcessingResult.Congestion;
  ...
  public ProcessingResult process(List<ReplicationTask> tasks) {
    ...
    try {
      ...
      if (!isSuccess(statusCode)) {
        if (statusCode == 503) {
          logger.warn(
              "Server busy (503) HTTP status code received from the peer {}; rescheduling tasks after delay",
              peerId);
          return ProcessingResult.Congestion;
        } else {
          // Unexpected error returned from the server. This should ideally never happen.
          logger.error(
              "Batch update failure with HTTP status code {}; discarding {} replication tasks",
              statusCode,
              tasks.size());
          return ProcessingResult.PermanentError;
        }
      ...
      if (maybeReadTimeOut(e)) {
        ...
        logger.error(
            "Not re-trying this exception because it does not seem to be a network exception", e);
        return ProcessingResult.PermanentError;
      }
    }
    return ProcessingResult.Success;
  ...
  /**
   * We want to retry eagerly, but without flooding log file with tons of error entries. As tasks
   * are executed by a pool of threads the error logging multiplies. For example: 20 threads * 100ms
  ...
  private void handleBatchResponse(
      List<ReplicationTask> tasks, List<ReplicationInstanceResponse> responseList) {
    if (tasks.size() != responseList.size()) {
      // This should ideally never happen unless there is a bug in the software.
      logger.error(
          "Batch response size different from submitted task list ({} != {}); skipping response analysis",
          responseList.size(),
          tasks.size());
      return;
    }
  ...
  private static boolean maybeReadTimeOut(Throwable e) {
    do {
      if (IOException.class.isInstance(e)) {
        String message = NullabilityUtil.castToNonnull(e.getMessage(), "can be null");
  ...
  private static ReplicationInstance createReplicationInstanceOf(InstanceReplicationTask task) {
    ...
    if (instanceInfo != null) {
      String overriddenStatus =
          task.getOverriddenStatus() == null ? null : task.getOverriddenStatus().name();
      ...
      String instanceStatus =
          instanceInfo.getStatus() == null ? null : instanceInfo.getStatus().name();

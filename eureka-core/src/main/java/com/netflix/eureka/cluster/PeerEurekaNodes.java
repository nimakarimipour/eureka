...
public class PeerEurekaNodes {
  ...
  protected List<String> resolvePeerUrls() {
    ...
    String zone =
        InstanceInfo.getZone(clientConfig.getAvailabilityZones(clientConfig.getRegion()), myInfo);
  ...
  /**
   * Given new set of replica URLs, destroy {@link PeerEurekaNode}s no longer available, and create
   * new ones.
   *
  ...
  protected PeerEurekaNode createPeerEurekaNode(String peerEurekaNodeUrl) {
    HttpReplicationClient replicationClient =
        JerseyReplicationClient.createReplicationClient(
            serverConfig, serverCodecs, peerEurekaNodeUrl);
   ...
   *     <p>Checks if the given service url contains the current host which is trying to replicate.
   *     Only after the EIP binding is done the host has a chance to identify itself in the list of
   *     replica nodes and needs to take itself out of replication traffic.
   * @param url the service url of the replica node that the check is made.
   * @return true, if the url represents the current node which is trying to replicate, false
   *     otherwise.
   */
  ...
  /**
   * Checks if the given service url contains the current host which is trying to replicate. Only
   * after the EIP binding is done the host has a chance to identify itself in the list of replica
   * nodes and needs to take itself out of replication traffic.
   *
   * @param url the service url of the replica node that the check is made.
   * @return true, if the url represents the current node which is trying to replicate, false
   *     otherwise.

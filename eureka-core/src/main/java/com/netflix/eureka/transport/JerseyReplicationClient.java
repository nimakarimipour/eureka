...
public class JerseyReplicationClient extends AbstractJerseyEurekaHttpClient
  ...
  /**
   * Compared to regular heartbeat, in the replication channel the server may return a more up to
   * date instance copy.
   */
  @Override
  public EurekaHttpResponse<InstanceInfo> sendHeartBeat(
      String appName, String id, InstanceInfo info, InstanceStatus overriddenStatus) {
    ...
    try {
      ...
      return anEurekaHttpResponse(response.getStatus(), infoFromPeer)
          .type(MediaType.APPLICATION_JSON_TYPE)
          .build();
    } finally {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "[heartbeat] Jersey HTTP PUT {}; statusCode={}",
            urlPath,
  ...
  @Override
  public EurekaHttpResponse<ReplicationListResponse> submitBatchUpdates(
    ...
    try {
      ...
      return anEurekaHttpResponse(response.getStatus(), batchResponse)
          .type(MediaType.APPLICATION_JSON_TYPE)
          .build();
    } finally {
      if (response != null) {
  ...
  public static JerseyReplicationClient createReplicationClient(
      EurekaServerConfig config, ServerCodecs serverCodecs, String serviceUrl) {
    ...
    try {
      ...
      if (serviceUrl.startsWith("https://")
          && "true"
              .equals(
                  System.getProperty(
                      "com.netflix.eureka.shouldSSLConnectionsUseSystemSocketFactory"))) {

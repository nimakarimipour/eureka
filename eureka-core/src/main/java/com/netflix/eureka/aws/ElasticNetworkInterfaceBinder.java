 ...
 *
 * <p>Candidate ENI's discovery is done using the same mechanism as Elastic ip binder, via dns
 * records or service urls.
 *
 * <p>The dns records and the service urls should use the ENI private dns or private ip
 *
 * <p>Dns record examples txt.us-east-1.eureka="us-east-1a.eureka" "us-east-1b.eureka"
 * txt.us-east-1a.eureka="ip-172-31-y-y.ec2.internal"
 * txt.us-east-1b.eureka="ip-172-31-x-x.ec2.internal" where "ip-172-31-x-x.ec2.internal" is the ENI
 * private dns
 *
 * <p>Service url example:
 * eureka.serviceUrl.us-east-1a=http://ip-172-31-x-x.ec2.internal:7001/eureka/v2/
 *
 * <p>ENI Binding strategy should be configured via property like:
 *
 * <p>eureka.awsBindingStrategy=ENI
 *
 * <p>If there are no available ENI's for the availability zone, it will not attach any already
 * attached ENI
 */
public class ElasticNetworkInterfaceBinder implements AwsBinder {
  ...
  public boolean alreadyBound() throws MalformedURLException {
    ...
    String myInstanceId =
        ((AmazonInfo) myInfo.getDataCenterInfo()).get(AmazonInfo.MetaDataKey.instanceId);
    AmazonEC2 ec2Service = getEC2Service();
    List<InstanceNetworkInterface> instanceNetworkInterfaces =
        instanceData(myInstanceId, ec2Service).getNetworkInterfaces();
    ...
    for (String ip : candidateIPs) {
      for (InstanceNetworkInterface ini : instanceNetworkInterfaces) {
        if (ip.equals(ini.getPrivateIpAddress())) {
          logger.info(
              "My instance {} seems to be already associated with the ip {}", myInstanceId, ip);
          return true;
        }
      }
    }
   ...
   *
   * <p>The candidate ENI's are deduced in the same wa the EIP binder works: Via dns records or via
   * service urls, depending on configuration.
   *
   * <p>It will try to attach the first ENI that is: Available For this subnet In the list of
   * candidate ENI's
   *
  ...
  public void bind() throws MalformedURLException {
    ...
    String myInstanceId =
        ((AmazonInfo) myInfo.getDataCenterInfo()).get(AmazonInfo.MetaDataKey.instanceId);
    String myZone =
        ((AmazonInfo) myInfo.getDataCenterInfo()).get(AmazonInfo.MetaDataKey.availabilityZone);

    final List<String> ips = getCandidateIps();
    Ordering<NetworkInterface> ipsOrder =
        Ordering.natural()
            .onResultOf(
                new Function<NetworkInterface, Integer>() {
    ...
    DescribeNetworkInterfacesResult result =
        ec2Service.describeNetworkInterfaces(
    ...
    if (result.getNetworkInterfaces().isEmpty()) {
      logger.info(
          "No ip is free to be associated with this instance. Candidate ips are: {} for zone: {}",
          ips,
          myZone);
    } else {
    ...
    for (InstanceNetworkInterface networkInterface : result) {
      if (ips.contains(networkInterface.getPrivateIpAddress())) {
        ...
        ec2.detachNetworkInterface(
            new DetachNetworkInterfaceRequest().withAttachmentId(attachmentId));
        break;
      }
    }
  }
  ...
  private Instance instanceData(String myInstanceId, AmazonEC2 ec2) {
    return ec2.describeInstances(new DescribeInstancesRequest().withInstanceIds(myInstanceId))
        .getReservations()
        .get(0)
        .getInstances()
        .get(0);
  ...
  /**
   * Based on shouldUseDnsForFetchingServiceUrls configuration, either retrieves candidates from dns
   * records or from configuration properties.
   */
      ...
      if (InetAddresses.isInetAddress(host)) {
        ...
        if (InetAddresses.isInetAddress(ip)) {
          ...
          throw new IllegalArgumentException(
              "Illegal internal hostname " + host + " translated to '" + ip + "'");
        }
      }
    }
    return ips;
    ...
    public void run() {
      ...
      try {
        ...
        if (alreadyBound) {
          ...
          timer.schedule(
              new IPBindingTask(), serverConfig.getEIPBindingRetryIntervalMsWhenUnbound());
        }
      }
    }
  }

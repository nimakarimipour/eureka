...
/**
 * An AWS specific <em>elastic ip</em> binding utility for binding eureka servers for a well known
 * <code>IP address</code>.
 *
 * <p><em>Eureka</em> clients talk to <em>Eureka</em> servers bound with well known <code>
 * IP addresses</code> since that is the most reliable mechanism to discover the <em>Eureka</em>
 * servers. When Eureka servers come up they bind themselves to a well known <em>elastic ip</em>
 *
 * <p>This binding mechanism gravitates towards one eureka server per zone for resilience. At least
 * one elastic ip should be slotted for each eureka server in a zone. If more than eureka server is
 * launched per zone and there are not enough elastic ips slotted, the server tries to pick a free
 * EIP slotted for other zones and if it still cannot find a free EIP, waits and keeps trying.
 *
...
public class EIPManager implements AwsBinder {
  ...
  public Collection<String> getCandidateEIPs(String myInstanceId, String myZone) {
    ...
    Collection<String> eipCandidates =
        clientConfig.shouldUseDnsForFetchingServiceUrls()
            ? getEIPsForZoneFromDNS(myZone)
            : getEIPsForZoneFromConfig(myZone);

    if (eipCandidates == null || eipCandidates.size() == 0) {
      throw new RuntimeException(
          "Could not get any elastic ips from the EIP pool for zone :" + myZone);
    }
   ...
   *
   * <p>This mechanism looks for the EIP pool in the zone the instance is in by looking up the DNS
   * name <code>{zone}.{region}.{domainName}</code>. The zone is fetched from the {@link
   * InstanceInfo} object;the region is picked up from the specified configuration {@link
   * com.netflix.discovery.EurekaClientConfig#getRegion()};the domain name is picked up from the
   * specified configuration {@link
   * com.netflix.discovery.EurekaClientConfig#getEurekaServerDNSName()} with a "txt." prefix (see
   ...
   * @return the collection of EIPs that exist in the zone this instance is in.
   */
  private Collection<String> getEIPsForZoneFromDNS(String myZone) {
    List<String> ec2Urls =
        EndpointUtils.getServiceUrlsFromDNS(
            clientConfig,
            myZone,
            true,
  ...
  /**
   * An EIP binding timer task which constantly polls for EIP in the same zone and binds it to
   * itself.If the EIP is taken away for some reason, this task tries to get the EIP back. Hence it
   * is advised to take one EIP assignment per instance in a zone.
   */
  private class EIPBindingTask extends TimerTask {
    ...
    public void run() {
      ...
      try {
        ...
        // If the EIP is not bound, the registry could  be stale. First sync up the registry from
        // the
        ...
        if (isEIPBound) {
          ...
          timer.schedule(
              new EIPBindingTask(), serverConfig.getEIPBindingRetryIntervalMsWhenUnbound());
        }
      }
    }
  }

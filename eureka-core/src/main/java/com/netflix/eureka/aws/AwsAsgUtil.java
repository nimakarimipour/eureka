...
public class AwsAsgUtil implements AsgClient {
  ...
  private ListeningExecutorService listeningCacheReloadExecutor =
      MoreExecutors.listeningDecorator(cacheReloadExecutor);

  // Cache for the AWS ASG information
  private final Timer timer = new Timer("Eureka-ASGCacheRefresh", true);
  private final com.netflix.servo.monitor.Timer loadASGInfoTimer =
      Monitors.newTimer("Eureka-loadASGInfo");
  ...
  @Inject
  public AwsAsgUtil(
      EurekaServerConfig serverConfig, EurekaClientConfig clientConfig, InstanceRegistry registry) {
        ...
        CacheBuilder.newBuilder()
            ...
            .build(
                new CacheLoader<CacheKey, Boolean>() {
                  ...
                  @Override
                  public ListenableFuture<Boolean> reload(final CacheKey key, Boolean oldValue)
                      throws Exception {
                    return listeningCacheReloadExecutor.submit(
  ...
  public boolean isASGEnabled(InstanceInfo instanceInfo) {
    CacheKey cacheKey =
        new CacheKey(getAccountId(instanceInfo, accountId), instanceInfo.getASGName());
    ...
    if (result != null) {
      ...
      // Only do an async refresh if it does not yet exist. Do this to refrain from calling aws api
      // too much
  ...
  /**
   * Check if the ASG is disabled. The amazon flag "AddToLoadBalancer" is queried to figure out if
   * it is or not.
   *
  ...
  private AutoScalingGroup retrieveAutoScalingGroupCrossAccount(String asgAccount, String asgName) {
    ...
    if (credentials == null
        || credentials.getExpiration().getTime() < System.currentTimeMillis() + 1000) {
      stsCredentials.put(asgAccount, initializeStsSession(asgAccount));
      credentials = stsCredentials.get(asgAccount);
    }

    ClientConfiguration clientConfiguration =
        new ClientConfiguration().withConnectionTimeout(serverConfig.getASGQueryTimeoutMs());
   ...
   *
   * @param asgAccountid the accountId this asg resides in, if applicable (null will use the default
   *     accountId)
   * @param asgName the name of the asg
   ...
   *
   * @return the long value representing the number of ASG queries done in the period.
   */
  @com.netflix.servo.annotations.Monitor(
      name = "numOfASGQueries",
   ...
   *
   * @return the long value representing the number of ASG queries that failed because of some
   *     reason.
   */
  @com.netflix.servo.annotations.Monitor(
      name = "numOfASGQueryFailures",
      ...
      public void run() {
        try {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
                "Trying to  refresh the keys for {}", Arrays.toString(cacheKeys.toArray()));
          }
   ...
   *
   * <p>The names are obtained from the {@link com.netflix.eureka.registry.InstanceRegistry} which
   * is then used for querying the AWS.
   *
  ...
  /**
   * Get the AWS account id where an ASG is created. Warning: This is expensive as it loops through
   * all instances currently registered.
   *
   * @param asgName The name of the ASG
  ...
  private String getASGAccount(String asgName) {
    ...
    logger.info(
        "Couldn't get the ASG account for {}, using the default accountId instead", asgName);
    return accountId;
  }
  ...
  private AmazonAutoScaling getAmazonAutoScalingClient() {
    ...
    if (null != aWSAccessId
      ...
      return new AmazonAutoScalingClient(
          new BasicAWSCredentials(aWSAccessId, aWSSecretKey), clientConfiguration);
    } else {
      return new AmazonAutoScalingClient(
          new InstanceProfileCredentialsProvider(), clientConfiguration);
    }
  }
  ...
  private static class CacheKey {
    ...
    public boolean equals(Object o) {
      ...
      if (asgAccountId != null
          ? !asgAccountId.equals(cacheKey.asgAccountId)
          : cacheKey.asgAccountId != null) return false;

...
public class Route53Binder implements AwsBinder {
  ...
  public Route53Binder(
    ...
    this(
        getRegistrationHostnameFromAmazonDataCenterInfo(applicationInfoManager),
        serverConfig,
        clientConfig,
  ...
  private static String getRegistrationHostnameFromAmazonDataCenterInfo(
      ApplicationInfoManager applicationInfoManager) {
    ...
    for (ResourceRecordSetWithHostedZone rrs : freeDomains) {
      if (createResourceRecordSet(rrs)) {
        logger.info(
            ...
            Nullability.castToNonnull(rrs.getResourceRecordSet(), "ensured not null").getName());
        return;
      }
    }
  ...
  private String getMyZone() {
    ...
    String zone =
        amazonInfo != null ? amazonInfo.get(AmazonInfo.MetaDataKey.availabilityZone) : null;
    if (zone == null) {
  ...
  private boolean executeChangeWithRetry(Change change, HostedZone hostedZone)
      throws InterruptedException {
  ...
  private ResourceRecordSetWithHostedZone getResourceRecordSetWithHostedZone(String domain) {
    ...
    if (hostedZone != null) {
      return new ResourceRecordSetWithHostedZone(
          hostedZone, getResourceRecordSet(domain, hostedZone));
    }
    return null;
  }
  ...
  private ResourceRecordSet getResourceRecordSet(String domain, HostedZone hostedZone) {
    ...
    ListResourceRecordSetsResult listResourceRecordSetsResult =
        amazonRoute53Client.listResourceRecordSets(request);
  ...
  private HostedZone getHostedZone(String domain) {
    ...
    ListHostedZonesResult listHostedZonesResult =
        amazonRoute53Client.listHostedZones(listHostedZoneRequest);
  ...
  private boolean hasValue(
    ...
    if (resourceRecordSetWithHostedZone != null
      ...
      for (ResourceRecord rr :
          resourceRecordSetWithHostedZone.getResourceRecordSet().getResourceRecords()) {

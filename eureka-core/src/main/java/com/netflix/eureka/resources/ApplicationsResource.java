...
public class ApplicationsResource {
   ...
   * @param appId the unique application identifier (which is the name) of the application.
   * @return information about a particular application.
   */
  @Path("{appId}")
  public ApplicationResource getApplicationResource(
      @PathParam("version") String version, @PathParam("appId") String appId) {
   ...
   * @param acceptEncoding the accept header to indicate whether to serve compressed or uncompressed
   *     data.
   ...
   * @param regionsStr A comma separated list of remote regions from which the instances will also
   *     be returned. The applications returned from the remote region can be limited to the
   *     applications returned by {@link EurekaServerConfig#getRemoteRegionAppWhitelist(String)}
  ...
  public Response getContainers(
    ...
    if (!isRemoteRegionRequested) {
      ...
      regions =
          Nullability.castToNonnull(regionsStr, "checked to be nonnull").toLowerCase().split(",");
      Arrays.sort(regions);
   ...
   *
   * <p>The delta changes represent the registry information change for a period as configured by
   * {@link EurekaServerConfig#getRetentionTimeInMSInDeltaQueue()}. The changes that can happen in a
   * registry include <em>Registrations,Cancels,Status Changes and Expirations</em>. Normally the
   * changes to the registry are infrequent and hence getting just the delta will be much more
   * efficient than getting the complete registry.
   *
   * <p>Since the delta information is cached over a period of time, the requests may return the
   * same data multiple times within the window configured by {@link
   * EurekaServerConfig#getRetentionTimeInMSInDeltaQueue()}.The clients are expected to handle this

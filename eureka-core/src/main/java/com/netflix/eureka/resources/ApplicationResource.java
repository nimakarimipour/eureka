...
public class ApplicationResource {
   ...
   * @param acceptHeader the accept header of the request to indicate whether to serve JSON or XML
   *     data.
  ...
  /**
   * Registers information about a particular instance for an {@link
   * com.netflix.discovery.shared.Application}.
   *
  ...
  public Response addInstance(
    ...
    if (isBlank(info.getId())) {
      ...
      return Response.status(400)
          .entity("Mismatched appName, expecting " + appName + " but was " + info.getAppName())
          .build();
    ...
    if (dataCenterInfo instanceof UniqueIdentifier) {
      ...
      if (isBlank(dataCenterInfoId)) {
        ...
        if (experimental) {
          ...
          logger.warn(
              "Registering DataCenterInfo of type {} without an appropriate id",

...
public class InstanceResource {
    ...
    InstanceResource(ApplicationResource app, @Nullable String id, EurekaServerConfig serverConfig, PeerAwareInstanceRegistry registry) {
        this.app = app;
        this.id = id;
     ...
     */
    @PUT
    public Response renewLease(@HeaderParam(PeerEurekaNode.HEADER_REPLICATION) String isReplication, @QueryParam("overriddenstatus") @Nullable String overriddenStatus, @QueryParam("status") @Nullable String status, @QueryParam("lastDirtyTimestamp") @Nullable String lastDirtyTimestamp) {
    ...
    @PUT
    @Path("status")
    public Response statusUpdate(@QueryParam("value") @Nullable String newStatus, @HeaderParam(PeerEurekaNode.HEADER_REPLICATION) String isReplication, @QueryParam("lastDirtyTimestamp") @Nullable String lastDirtyTimestamp) {
        try {
    ...
    @DELETE
    @Path("status")
    public Response deleteStatusUpdate(@HeaderParam(PeerEurekaNode.HEADER_REPLICATION) String isReplication, @QueryParam("value") @Nullable String newStatusValue, @QueryParam("lastDirtyTimestamp") String lastDirtyTimestamp) {
        try {
            if (registry.getInstanceByAppAndId(app.getName(), id) == null) {
                logger.warn("Instance not found: {}/{}", app.getName(), id);
                return Response.status(Status.NOT_FOUND).build();

 ...
 *
 * <p>The eureka server is configured by using the configuration {@link EurekaServerConfig}
 * specified by <em>eureka.server.props</em> in the classpath. The eureka client component is also
 * initialized by using the configuration {@link EurekaInstanceConfig} specified by
 * <em>eureka.client.props</em>. If the server runs in the AWS cloud, the eureka server binds it to
 * the elastic ip as specified.
 *
...
public class EurekaBootStrap implements ServletContextListener {
  ...
  protected void initEurekaServerContext() throws Exception {
    ...
    JsonXStream.getInstance()
        .registerConverter(new V1AwareInstanceInfoConverter(), XStream.PRIORITY_VERY_HIGH);
    ...
    if (isAws(applicationInfoManager.getInfo())) {
      registry =
          new AwsInstanceRegistry(
              eurekaServerConfig, eurekaClient.getEurekaClientConfig(), serverCodecs, eurekaClient);
      awsBinder =
      ...
      registry =
          new PeerAwareInstanceRegistryImpl(
              eurekaServerConfig, eurekaClient.getEurekaClientConfig(), serverCodecs, eurekaClient);
    }
    ...
    serverContext =
        new DefaultEurekaServerContext(
            eurekaServerConfig, serverCodecs, registry, peerEurekaNodes, applicationInfoManager);
  ...
  protected PeerEurekaNodes getPeerEurekaNodes(
      PeerAwareInstanceRegistry registry,
    ...
    PeerEurekaNodes peerEurekaNodes =
        new PeerEurekaNodes(
            registry, eurekaServerConfig, eurekaClientConfig, serverCodecs, applicationInfoManager);

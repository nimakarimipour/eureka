 ...
 * of such rules and pick the first result that matches.
 *
 * <p>It is designed to be used by {@link
...
public interface InstanceStatusOverrideRule {
   ...
   * @param existingLease Does the instance have an existing lease already? If so let's consider
   *     that.
   * @param isReplication When overriding consider if we are under a replication mode from other
   *     servers.

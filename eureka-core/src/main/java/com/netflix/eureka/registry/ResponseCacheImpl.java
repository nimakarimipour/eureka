...
/**
 * The class that is responsible for caching registry information that will be queried by the
 * clients.
 *
 * <p>The cache is maintained in compressed and non-compressed form for three categories of requests
 * - all applications, delta changes and for individual applications. The compressed form is
 * probably the most efficient in terms of network traffic especially when querying all
 * applications.
 *
 * <p>The cache also maintains separate pay load for <em>JSON</em> and <em>XML</em> formats and for
...
public class ResponseCacheImpl implements ResponseCache {
  ...
  private final Timer serializeAllAppsWithRemoteRegionTimer =
      Monitors.newTimer("serialize-all_remote_region");
  private final Timer serializeDeltaAppsWithRemoteRegionTimer =
      Monitors.newTimer("serialize-all-delta_remote_region");
   ...
   * clients) Since, during invalidation, triggered by a change in registry for local region, we do
   * not know the regions requested by clients, we use this mapping to get all the keys with regions
   * to be invalidated. If we do not do this, any cached user requests containing region keys will
   * not be invalidated and will stick around till expiry. Github issue:
      ...
      public void run() {
        ...
        for (Key key : readOnlyCacheMap.keySet()) {
          ...
          try {
            ...
            logger.error(
                "Error while updating the client cache from response cache for key {}",
                key.toStringCompact(),
                th);
          } finally {
   ...
   *
   * <p>If the cached information is not available it is generated on the first request. After the
   * first request, the information is then updated periodically by a background thread.
   *
  ...
  public void invalidate(Key... keys) {
    for (Key key : keys) {
      logger.debug(
          "Invalidating the response cache key : {} {} {} {}, {}",
          key.getEntityType(),
          key.getName(),
          key.getVersion(),
          key.getType(),
   ...
   *     <p>Gets the version number of the cached data.
   * @return teh version number of the cached data.
   */
  @Deprecated
   ...
   *     <p>Gets the version number of the cached data with remote regions.
   * @return teh version number of the cached data with remote regions.
   */
  @Deprecated
  ...
  private String getPayLoad(Key key, Applications apps) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "New application cache entry {} with apps hashcode {}",
          key.toStringCompact(),
        ...
        case Application:
          ...
          if (ALL_APPS.equals(key.getName())) {
            if (isRemoteRegionRequested) {
              ...
              payload =
                  getPayLoad(key, registry.getApplicationsFromMultipleRegions(key.getRegions()));
            } else {
            ...
            if (isRemoteRegionRequested) {
              ...
              payload =
                  getPayLoad(
                      key, registry.getApplicationDeltasFromMultipleRegions(key.getRegions()));
            } else {

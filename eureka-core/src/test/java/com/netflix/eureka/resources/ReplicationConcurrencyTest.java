...
/**
 * A pseudo mock test to test concurrent scenarios to do with registration and replication of
 * cluster setups with 1+ eureka servers
 *
 * @author David Liu
 */
public class ReplicationConcurrencyTest {
  ...
  public void setUp() throws Exception {
    ...
    Mockito.when(server1Peers.getPeerEurekaNodes())
    ...
    Mockito.when(server2Peers.getPeerEurekaNodes())
        .thenReturn(Collections.<PeerEurekaNode>emptyList());
  ...
  /**
   * this test tests a scenario where multiple registration and update requests for a single client
   * is sent to different eureka servers before replication can occur between them
   */
  @Test
  public void testReplicationWithRegistrationAndUpdateOnDifferentServers() throws Exception {
    ...
    // without batching this is done by server1 making a REST call to the register endpoint of
    // server2 with
    // replication=true
  ...
  private static class MockServer {
    ...
    public MockServer(String appName, PeerEurekaNodes peerEurekaNodes) throws Exception {
      ApplicationInfoManager infoManager =
          new ApplicationInfoManager(new MyDataCenterInstanceConfig());
      ...
      Mockito.doReturn("true")
          .when(serverConfig)
          .getExperimental("registry.registration.ignoreIfDirtyTimestampIsOlder");

      this.registry =
          new PeerAwareInstanceRegistryImpl(serverConfig, clientConfig, serverCodecs, eurekaClient);

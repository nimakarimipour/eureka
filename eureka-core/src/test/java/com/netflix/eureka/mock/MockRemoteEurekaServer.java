...
public class MockRemoteEurekaServer extends ExternalResource {
  ...
  public MockRemoteEurekaServer(
    ...
    handler
        .addFilterWithMapping(ServerRequestAuthFilter.class, "/*", 1)
        .setFilter(new ServerRequestAuthFilter(serverContext));
    handler
        .addFilterWithMapping(RateLimitingFilter.class, "/*", 1)
        .setFilter(new RateLimitingFilter(serverContext));
  ...
  private static String stringifyAppMap(Map<String, Application> applicationMap) {
    ...
    for (Map.Entry<String, Application> entry : applicationMap.entrySet()) {
      String entryAsString =
          String.format(
              "{ name : %s , instance count: %d }",
              entry.getKey(), entry.getValue().getInstances().size());
  ...
  private class AppsResourceHandler extends ServletHandler {
    ...
    public void handle(
      ...
      System.out.println(
          "Eureka resource mock, received request on path: "
              + pathInfo
              + ". HTTP method: |"
              + request.getMethod()
              + '|');
      ...
      if (null != pathInfo && pathInfo.startsWith("")) {
        ...
        if (pathInfo.startsWith("apps/delta")) {
          ...
          InstanceInfo instanceInfo =
              InstanceInfo.Builder.newBuilder().setAppName("TEST-APP").build();
          sendOkResponseWithContent(
              (Request) request,
              response,
              new EurekaJsonJacksonCodec()
                  .getObjectMapper(Applications.class)
    ...
    private void sendOkResponseWithContent(
        Request request, HttpServletResponse response, String content) throws IOException {
      ...
      System.out.println(
          "Eureka resource mock, sent response for request path: "
              + request.getPathInfo()
              + " with content"
              + content);
    }
  ...
  private String toJson(Applications apps) throws IOException {
    return new EurekaJsonJacksonCodec()
        .getObjectMapper(Applications.class)
        .writeValueAsString(apps);
  }
}

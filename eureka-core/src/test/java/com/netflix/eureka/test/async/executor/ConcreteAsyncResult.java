...
public class ConcreteAsyncResult<T> implements AsyncResult<T> {
  ...
  /**
   * Sets the result and unblocks all threads waiting on {@link #get()} or {@link #get(long,
   * TimeUnit)}.
   *
  ...
  /**
   * Gets the value of the result in blocking way. Using {@link #get()} or {@link #get(long,
   * TimeUnit)} is usually preferred because these methods block until the result is available or an
   * error occurs.
   *
   ...
   * {@link #get(long, TimeUnit)} should be called first because these methods block until execution
   * has completed.
   *
   * @return the error that was thrown, or null if no error has occurred or if the execution has not
   *     yet completed.
   */

...
public class AsyncSequentialExecutor {
  ...
  /**
   * Result status, if events are executed successfully in sequential manner, then return this by
   * default
   */
   ...
   * If calling is successful, then will return {@link ResultStatus#DONE}, or else exception will be
   * thrown and {@link AsyncResult} will be filled with the error.
   *
  ...
  protected <T> AsyncResult<T> run(Callable<T> task) {
    ...
    new Thread(
            ...
            "AsyncSequentialExecutor-" + INDEX.incrementAndGet())
        .start();
    return result;
  }
}

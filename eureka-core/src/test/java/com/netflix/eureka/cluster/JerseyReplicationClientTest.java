...
/**
 * Ideally we would test client/server REST layer together as an integration test, where server side
 * has mocked service layer. Right now server side REST has to much logic, so this test would be
 * equal to testing everything. Here we test only client side REST communication.
 *
 * @author Tomasz Bak
 */
public class JerseyReplicationClientTest {

  @Rule public MockServerRule serverMockRule = new MockServerRule(this);
    ...
    serverMockClient
        .when(
            request()
                ...
                .withHeader(header(PeerEurekaNode.HEADER_REPLICATION, "true"))
                .withPath(
                    "/eureka/v2/apps/" + instanceInfo.getAppName() + '/' + instanceInfo.getId()))
        .respond(response().withStatusCode(204));

    EurekaHttpResponse<Void> response =
        replicationClient.cancel(instanceInfo.getAppName(), instanceInfo.getId());
  ...
  public void testHeartbeatReplicationWithNoResponseBody() throws Exception {
    ...
    EurekaHttpResponse<InstanceInfo> response =
        replicationClient.sendHeartBeat(
            instanceInfo.getAppName(), instanceInfo.getId(), instanceInfo, InstanceStatus.DOWN);
    assertThat(response.getStatusCode(), is(equalTo(200)));
  ...
  public void testHeartbeatReplicationWithResponseBody() throws Exception {
    ...
    EurekaHttpResponse<InstanceInfo> response =
        replicationClient.sendHeartBeat(
            this.instanceInfo.getAppName(), this.instanceInfo.getId(), this.instanceInfo, null);
  ...
  public void testAsgStatusUpdateReplication() throws Exception {
    ...
    EurekaHttpResponse<Void> response =
        replicationClient.statusUpdate(instanceInfo.getASGName(), ASGStatus.ENABLED);
    assertThat(response.getStatusCode(), is(equalTo(200)));
  }
  ...
  public void testDeleteStatusOverrideReplication() throws Exception {
    ...
    EurekaHttpResponse<Void> response =
        replicationClient.deleteStatusOverride(

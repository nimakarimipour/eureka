...
public class TestableHttpReplicationClient implements HttpReplicationClient {
  ...
  public HandledRequest nextHandledRequest(long timeout, TimeUnit timeUnit)
  ...
  public EurekaHttpResponse<InstanceInfo> sendHeartBeat(
    ...
    return anEurekaHttpResponse(statusCode, instanceInfoFromPeer)
  ...
  @Override
  public EurekaHttpResponse<Void> statusUpdate(
      String appName, String id, InstanceStatus newStatus, InstanceInfo info) {
    handledRequests.add(new HandledRequest(RequestType.StatusUpdate, newStatus));
    return EurekaHttpResponse.status(networkStatusCodes[callCounter.getAndIncrement()]);
  }

  @Override
  public EurekaHttpResponse<Void> deleteStatusOverride(
  ...
  public EurekaHttpResponse<ReplicationListResponse> submitBatchUpdates(
    ...
    int statusCode = networkStatusCodes[callCounter.getAndIncrement()];
    return anEurekaHttpResponse(statusCode, replicationListResponse)
        .type(MediaType.APPLICATION_JSON_TYPE)
        .build();
  }
